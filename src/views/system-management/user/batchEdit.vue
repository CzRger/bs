...
<script>
  ...
  export default {
    ...
    activated() {
      // 禁止暴露admin角色
      RoleAPI.queryAllRoles({filters: [{type: 'NE', field: 'id', value: 'admin'}]}).then(data => {
        this.roles = data
      })
      ...
      this.allUserGroups = []
      GroupAPI.queryAllGroups({filters: [{field: 'category', value: 1}]}).then(data => {
        this.allUserGroups = data
      }) // 系统已有的所有用户组
      this.allRoleGroups = []
      GroupAPI.queryAllGroups({filters: [{field: 'category', value: 2}]}).then(data => {
    ...
    methods: {
      ...
      submitUserRoleHandler() {
        ...
        this.$confirm('此操作将批量授权, 是否继续?', '提示', {
          ...
          UserRoleAPI.reset({ entities: userRoles, mappingFields: ['userId']}).then(() => {
      ...
      submitUserDeptHandler() {
        ...
        this.$confirm('此操作将批量授权, 是否继续?', '提示', {
          ...
          UserDeptAPI.reset({ entities: userDepts, mappingFields: ['userId']}).then(() => {
            this.optionSuccessHandler()
          })
        }).catch(() => {
          this.$message({
      ...
      submitUserGroupHandler() {
        ...
        this.$confirm('此操作将批量授权, 是否继续?', '提示', {
          ...
          UserGroupAPI.reset({ entities: userGroups, mappingFields: ['userId']}).then(() => {
            this.optionSuccessHandler()
          })
        }).catch(() => {
          this.$message({
      ...
      submitUserRoleGroupHandler() {
        ...
        this.$confirm('此操作将批量授权, 是否继续?', '提示', {
          ...
          UserRoleGroupAPI.reset({ entities: userRoleGroups, mappingFields: ['userId']}).then(() => {
            this.optionSuccessHandler()
          })
        }).catch(() => {
          this.$message({
...
<style lang="scss" scoped>
  ...
  /deep/ .el-tree {
    border-bottom: 1px solid $element-border-color-base;

    .el-tree-node__content {
      border-top: 1px solid $element-border-color-base;
      border-left: 1px solid $element-border-color-base;
      border-right: 1px solid $element-border-color-base;
      min-height: 40px;
    }
  }
