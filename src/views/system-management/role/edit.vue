...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      resetRoleMenus() {
        const nodes = this.$refs['menusTree'].getCheckedNodes(false, true)
        const data = []
        nodes.forEach(node => {
          data.push({roleId: this.detail.id, menuId: node.id})
        })
        MenuRoleAPI.reset({ entities: data, mappingFields: ['roleId']}).then(this.optionSuccessHandler)
      },
