<template>
  <el-row>
      ...
      <button-right>
        ...
        <template slot="button">
          <el-button-group>
            ...
            <el-button v-if="selected && selected.isLeaf == 'NO'" type="primary" @click="$emit('option-changed','add', selected)">新增下级</el-button>
    ...
    <el-col :span="24">
      ...
      <el-tree ref="tree" :data="pagination.list" :load="loadChildren" :props="defaultProps"
               :filter-node-method="filterNodeHandler" node-key="id" class="filter-tree" highlight-current accordion
               lazy @current-change="(value, node) => selected = value">
    ...
    data() {
      ...
      return {
        ...
        defaultProps: {
          children: 'children',
          label: 'name',
          isLeaf: (data, node) => data.id && node.data.isLeaf === 'YES'
        },
        filter: ''
      }
    },
    ...
    methods: {
      ...
      loadChildren(node, resolve) {
        ...
        DictionaryAPI.queryAllDictionaries({
          filters: [{field: 'category', value: 3}, {
            field: 'parentId',
            value: node.data.id
          }]
        }).then(data => {
