    ...
    <el-collapse value="base-info" accordion>
      <el-collapse-item title="基本信息" name="base-info">
        <el-form :model="detail" :label-width="labelWidth">
          ...
          <input-item-view v-if="detail.category == '3'" label="字典层级">{{ detail.level }}</input-item-view>
          <input-item-view v-if="detail.category == '3'" label="是否为叶子节点">{{ detail.isLeaf }}</input-item-view>
